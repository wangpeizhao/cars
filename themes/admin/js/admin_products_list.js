function setData(currPage) {
    try {
        $.ajax({
            type: "POST",
            url: baseUrl+lang + '/admin/system/products',
            data: {
                currPage: currPage,
                rows: rows,
                condition: condition
...
function fillData(data, currPage) {
    try {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            html += '	<td>' + (data[i]['image_url'] ? '<a href="' + site_url + data[i]['image_url'] + '" target="_blank" style="color:#339900;">[图]</a>': '') + '<a href="' + baseUrl + lang + '/products/info/' + data[i]['id'] + '.html' + '" target="_blank">' + data[i]['title'] + '</a>' + '</td>';
