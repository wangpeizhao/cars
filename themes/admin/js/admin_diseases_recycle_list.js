function setData(currPage) {
    try {
        $.ajax({
            type: "POST",
            url: baseUrl+lang + '/admin/diseases/recycle',
            data: {
                currPage: currPage,
                rows: rows,
                condition: condition
...
function fillData(data, currPage) {
    try {
        ...
        for (var i = 0; i < data.length; i++) {
			...
			html += '	<td><a href="' + baseUrl + lang + '/diseases/sort/' + data[i]['slug'] + '" target="_blank">' + data[i]['term_name'] + '</a></td>';
			html += '	<td><a href="' + baseUrl + lang + '/diseases/info/' + data[i]['id'] + '.html' + '" target="_blank" title="' + data[i]['title'] + '">' + data[i]['title'] + '</a>' + '</td>';
