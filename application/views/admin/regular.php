  ...
  <!-- /引入二级菜单-->
	...
	$(function(){
		try{
			...
			$(".saveRegular").click(function(){
				...
				var role_name = roleId!='-1'?$('select option:selected').text():'';
				if(role_name){
	...
	$(function(){
		...
		$('select[name="roleId"]').change(function(){
			var val = $(this).val();
			event_link(baseUrl + lang +'/admin/premier/regular?roleId='+val);
		});
	});

	function doRegular(regular){
		$.post(baseUrl + lang + "/admin/premier/regular",{regular:regular,roleId:$('select[name="roleId"]').val(),role_name:$('input[name="role_name"]').val()}, function(data){
			if (data.code === 1) {
				alert('提交成功,下次登录生效');
				event_link(baseUrl + lang +'/admin/premier/regular?roleId='+data.msg);
			}else if(data.msg){
				alert(data.msg);
				return false;
			}else{
	...
	<style type="text/css">
		...
		.selected i{
			position: absolute;bottom:5px;right: 0px;font-size: 12px;color: #FFF;cursor: pointer;background: #20a3fe;width: 14px;height: 14px;line-height: 14px;
		...
		<table class="form_table" style="margin-top:0px;">
			...
			<tbody>
			  <tr>
				<th>角色分类：</th>
				<td>
				  <select name="roleId" class="roles">
					<option value="">- 请选择角色 -</option>
					<option value="-1"<?=$roleId == -1?' selected':''?>>- 添加新角色 -</option>
					<?php if($roles){
						foreach($roles as $key=>$item){
							if(!$item['supervisor']){?>
					<option value="<?=$item['id']?>"<?=$roleId == $item['id']?' selected':''?>><?=$item['role_name']?></option>
					<?php }}}?>
				  </select> <input type="text" name="role_name" placeholder="请输入新的角色名称" class="dn normal" style="height: 28px;" value=""><font color="#ff6600">(超级管理员拥有所有权限) - <font color="#0066ff">(下次登录生效，包括当前用户)</font></font>
				</td>
			  </tr>
			  <tr>
				<th>展开/收起：</th>
