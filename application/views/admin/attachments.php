...
<!--
		...
		try{
			...
			$(".addLinkBtn").toggle(
				function () {
					...
					$(".addLinks form").attr('action',baseUrl + lang +'/admin/attachments/add');
				},
				function () {
			...
			$(".seeimg a#delImg").live('click',function(){
				if(confirm('确定要删除该友情链接图片！')){
					...
					$.post(baseUrl + lang + "/admin/attachments/edit",{imgPath:imgPath,id:link_id,act:'delLinkImage'}, function(data){
						if (data.done === true) {
							alert('删除成功');
							$(".seeimg").html('');
	...
	function click_list_table_a(obj){
		try{
			...
			switch(act){
				case 'edit':
					$.post(baseUrl + lang + "/admin/attachments/edit",{id:id,act:'get'}, function(data){
						if(data.code=='1'){
				...
				case 'del':
					if(confirm('确定要把'+(_TITLE_?_TITLE_:'信息')+'放到回收站内？')){
						$.post(baseUrl + lang + "/admin/attachments/del",{id:id}, function(data){
							if (data.done === true) {
								alert('删除成功');
