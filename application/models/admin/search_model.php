 ...
 */
class Search_model extends Fzhao_Model {
     ...
     * 时间：2014-1-21
     */
    function get_list($data, $is_valid = 1,$type = 'antifake') {
        ...
        $data = $this->getData(array_merge(array(
            'fields' => 'p.id,p.term_id,p.title,p.is_valid,p.owner,p.views,p.is_issue,p.create_time,p.update_time,t.name term_name,t.slug,a.username',
            'table' => $this->table . ' p',
     ...
     * 时间：2012-11-11
     */
    function get_recycle_list($data,$type='antifake') {
        return $this->get_list($data, 0, $type);
    }
