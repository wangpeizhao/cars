...
class Diseases extends Fzhao_Controller{

	function __construct(){
		parent::__construct();
		$this->checkLP();//检测登录状态和权限
		$this->load->library('uri');
		$this->load->model('admin/diseases_model');
		$this->model = $this->diseases_model;
	}
	...
	function index(){
		if(IS_POST){
			...
			$data = array();
			$data['terms'] = $this->model->getTermByTaxonomy('diseases');
			$this->load->view('admin/diseases',$data);
		}
	}
	...
	function add(){
		if(IS_POST){
			...
			$data['terms'] = $this->model->getTermByTaxonomy('diseases');
			$this->load->view('admin/diseasesAdd',$data);
		}
	}
	...
	function edit($id=null){
		if(IS_POST){
			...
			$result = $this->model->dbUpdate('diseases',$data,array('id'=>$id));
			$this->doIframe($result);
		}else{
			...
			$data['terms'] = $this->model->getTermByTaxonomy('diseases');
			$data['data'] = $service;
			$this->load->view('admin/diseasesEdit',$data);
		}
	}
	...
	function recycle(){
		if(IS_POST){
			...
			$data = array();
			$data['terms'] = $this->model->getTermByTaxonomy('diseases');
			$this->load->view('admin/diseasesRecycle',$data);
		}
	}
